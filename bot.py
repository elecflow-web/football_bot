...
class TeamAnalyzer:
    ...
    def _load_team_stats(self) -> Dict:
        ...
        # В реальной системе это будет из Football-Data.org API
        # Для примера используем симуляцию с реальными данными
    ...
    def _calculate_form(self) -> float:
        ...
        # REAL: запрос к https://api.football-data.org
        # Берём последние 10 матчей и считаем точки
        # Победа = 3 точки, Ничья = 1, Поражение = 0
        
        # Симуляция: форма от -30 до +30
        return random.uniform(-0.15, 0.25)
    
    def _calculate_home_away(self) -> Dict:
        """Анализирует игру дома и в гостях"""
        return {
            'home_wins_pct': random.uniform(0.30, 0.70),  # % побед дома
            'away_wins_pct': random.uniform(0.15, 0.55),  # % побед в гостях
    ...
    def _get_injuries(self) -> List[Dict]:
        ...
        # REAL: запрос к Transfermarkt или WhoScored
        injuries = []
        
        # 20% вероятность что есть серьёзная травма
        if random.random() < 0.20:
            injuries.append({
                'player': f'Key Player {self.team_name}',
                'position': 'Defender',  # Защитник = -10% к результату
                'impact': -0.10
            })
    ...
    def _get_recent_matches(self) -> List[Dict]:
        ...
        # REAL: запрос к Football-Data.org API
        return [
    ...
    def get_probability_adjustments(self) -> Dict:
        ...
        adjustments = {
            'form': self.stats['form'] * 0.12,  # Форма: ±12%
...
class MatchAnalyzer:
    ...
    def analyze_match(self) -> Dict:
        ...
        # Корректировки домашней команды
        ...
        normalized_home = (implied_home / total) * 0.97  # Убираем ~3% маржу
...
if __name__ == "__main__":
    ...
    for i, bet in enumerate(bets, 1):
        ...
        print(f"   ⚡ EDGE (преимущество): {bet['edge']*100:.1f}%")
