...
def format_market_type(market: str, market_type: str) -> str:
    """Форматирует описание рынка для понимания"""
    descriptions = {
        "Over": "ТОТАЛ БОЛЬШЕ (Over) - в матче будет больше голов",
        "Under": "ТОТАЛ МЕНЬШЕ (Under) - в матче будет меньше голов",
    }
...
async def top_bets_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for i, bet_data in enumerate(bets[:12], 1):
            ...
            market_description = format_market_type(market, market_type)
        ...
        text += (
            ...
            "   • Over 2.5 = будет 3+ голов\n"
            "   • Under 2.5 = будет 0, 1 или 2 гола\n\n"
            "🟢 *Фора (Гандикап)* - команда играет с минусом\n"
            "   • Фора -1.5 = команда должна выиграть с разницей 2+ гола\n"
            "   • Фора +1.5 = команде даётся +1.5 гола\n\n"
            "🟢 *П1/П2/X* - исход матча\n"
            "   • П1 = победа дома\n"
            "   • П2 = победа в гостях\n"
            "   • X = ничья\n\n"
